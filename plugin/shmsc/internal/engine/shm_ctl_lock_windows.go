...
func acquireControlLock(ctx context.Context, ctlName string) (func(), error) {
	...
	// A single reusable timer rather than time.After per iteration: time.After
	// cannot be stopped early, so an entry that leaves the select via ctx.Done
	// would leak its timer until the interval elapsed.
