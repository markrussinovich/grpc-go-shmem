...
// Package main implements a gRPC server over shared memory that demonstrates all RPC types.
//
// It implements the route guide service whose definition can be found in routeguide/route_guide.proto.
// This version uses shared memory transport instead of TCP.
package main
...
var (
	...
	shmName     = flag.String("shm_name", "routeguide_shm", "Shared memory segment name")
...
func main() {
	...
	if err != nil {
		log.Fatalf("failed to create shared memory listener: %v", err)
	}
	defer lis.Close()
	...
	log.Printf("Server listening on shm://%s", *shmName)
...
// exampleData is the default set of features if no JSON file is provided
var exampleData = []byte(`[{
	...
	"name": "Travis Wolfe Park, Randolph, NJ, USA"
}]`)
