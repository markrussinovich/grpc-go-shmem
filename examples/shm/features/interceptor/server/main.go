...
var (
	addr = flag.String("addr", "shm://interceptor_shm", "the address to listen on")
...
func main() {
	...
	// Extract segment name from shm:// URL
	...
	// For shm transport, we don't need TLS (it's local IPC) but we still
	// use interceptors for auth token validation.
