...
// service stubs over shared memory transport.
package main
...
var addr = flag.String("addr", "shm://multiplex_shm", "the address to connect to")
...
func main() {
	...
	fmt.Println()
	fmt.Println("--- calling grpc.examples.echo.Echo/UnaryEcho ---")
	// Make an echo client with the same ClientConn.
	ecc := ecpb.NewEchoClient(conn)
	callUnaryEcho(ecc, "this is examples/multiplex")
}
