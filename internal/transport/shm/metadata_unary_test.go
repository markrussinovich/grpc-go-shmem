...
func TestUnary_MetadataAndStatus(t *testing.T) {
    ...
    go func() {
        ...
        if err != nil { t.Errorf("decode headers: %v", err); return }
        if hdr.HdrType != 0 || hdr.Method != "/svc.M/Echo" || hdr.Authority != "example.com" {
            t.Errorf("unexpected request hdr: %+v", hdr)
            return
        }
        if hdr.DeadlineUnixNano == 0 { t.Errorf("missing deadline hint") }
        ...
        if err := writeFrame(srvTx, FrameHeader{StreamID: fh.StreamID, Type: FrameTypeTRAILERS, Flags: TrailersFlagEND_STREAM}, encodeTrailers(tr)); err != nil {
            t.Errorf("write trailers: %v", err); return
        }
    }()
    ...
    payload[0] = 0; payload[1] = 4; payload[2] = 0; payload[3] = 0; payload[4] = 0
    ...
    if len(msg) != len(payload) { t.Fatalf("payload len mismatch: got=%d want=%d", len(msg), len(payload)) }
    for i := range msg { if msg[i] != payload[i] { t.Fatalf("payload mismatch at %d", i) } }
...
func mdEqual(a, b []KV) bool {
    ...
    for i := range a {
        ...
        if len(a[i].Values) != len(b[i].Values) { return false }
        for j := range a[i].Values { if string(a[i].Values[j]) != string(b[i].Values[j]) { return false } }
    }
    return true
}
...
func mdContains(have []KV, want []KV) bool {
    ...
    for _, w := range want {
        found := false
        for _, h := range have {
            if h.Key != w.Key { continue }
            if len(h.Values) != len(w.Values) { continue }
            eq := true
            for i := range h.Values {
                if string(h.Values[i]) != string(w.Values[i]) { eq = false; break }
            }
