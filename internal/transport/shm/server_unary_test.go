...
func TestShmServerUnary_Echo(t *testing.T) {
    ...
    if err != nil { t.Fatalf("NewURLListener: %v", err) }
    defer lis.Close()
    ...
    sc, ok := conn.(*shmConn)
    if !ok { t.Errorf("unexpected conn type"); return }
    st := sc.transport
    ...
    if hdr.HdrType != 1 { t.Fatalf("expected server-initial, got %v", hdr.HdrType) }
    if tr.GRPCStatusCode != 0 { t.Fatalf("expected status OK, got %d", tr.GRPCStatusCode) }
    if len(msg) != len(payload) { t.Fatalf("len mismatch: %d vs %d", len(msg), len(payload)) }
