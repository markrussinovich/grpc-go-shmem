...
func TestShmUnaryClient_Echo(t *testing.T) {
	...
	go func() {
		...
		if err := writeFrame(srvTx, FrameHeader{StreamID: fh.StreamID, Type: FrameTypeHEADERS, Flags: HeadersFlagINITIAL}, encodeHeaders(h), context.Background()); err != nil {
			t.Errorf("server write headers: %v", err)
			return
		}
		// Echo MESSAGE
		if err := writeFrame(srvTx, FrameHeader{StreamID: fh.StreamID, Type: FrameTypeMESSAGE}, msg, context.Background()); err != nil {
			t.Errorf("server write message: %v", err)
			return
		}
		// Write TRAILERS OK
		tr := TrailersV1{Version: 1, GRPCStatusCode: 0}
		if err := writeFrame(srvTx, FrameHeader{StreamID: fh.StreamID, Type: FrameTypeTRAILERS, Flags: TrailersFlagEND_STREAM}, encodeTrailers(tr), context.Background()); err != nil {
	...
	if hdr.HdrType != 1 {
		t.Fatalf("expected server-initial headers, got %v", hdr.HdrType)
	}
	// Verify echoed metadata
	found := false
	for _, kv := range hdr.Metadata {
		if kv.Key == "x-reply" && len(kv.Values) == 1 && string(kv.Values[0]) == "yes" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected x-reply metadata in headers")
	}
	if tr.GRPCStatusCode != 0 {
		t.Fatalf("expected OK status, got %d", tr.GRPCStatusCode)
	}
	if len(msg) != len(payload) {
		t.Fatalf("echo length mismatch: got %d want %d", len(msg), len(payload))
	}
	for i := range msg {
		if msg[i] != payload[i] {
			t.Fatalf("echo data mismatch at %d", i)
		}
	}
