...
const (
    ...
    DefaultRingASize   = 64 * 1024 * 1024   // 64 MiB client→server
    DefaultRingBSize   = 64 * 1024 * 1024   // 64 MiB server→client
...
| `spaceSeq` | Reader | Writers | Signals "full→not-full" transition |
| `contigSeq` | Reader | Writers | Signals "any read completed" |

### Adaptive Spin-Wait

Before falling back to futex (which has ~7-10µs wake overhead), the transport performs a brief spin-wait:

```go
const (
    spinIterationsDefault = 300  // ~2µs at 7ns/PAUSE
...
func (b *shmResolverBuilder) Build(target resolver.Target, ...) (resolver.Resolver, error) {
    // Parse "shm://segment_name" → address "shm:segment_name"
...
└─────┬──────┘                              └─────┬──────┘
      │                                           │
      │  1. Open control segment                  │
      │  ─────────────────────────────────────────│
      │                                           │
      │  2. Send CONNECT frame                    │
      │  ──────────────────────────────────────►  │
      │                                           │
      │  3. Server creates data segment           │
      │                                           │
      │  4. Send ACCEPT frame (segment name)      │
      │  ◄──────────────────────────────────────  │
      │                                           │
      │  5. Open data segment                     │
      │  ─────────────────────────────────────────│
      │                                           │
      │  6. Set clientReady flag                  │
      │  ─────────────────────────────────────────│
      │                                           │
      │  7. Begin RPC communication               │
      │  ◄─────────────────────────────────────►  │
```
...
BenchmarkShmRingWriteRead/size=64KB   256K ops  4.2 µs/op   15,520 MB/s
BenchmarkShmRingWriteRead/size=1MB     25K ops   47 µs/op   22,142 MB/s
```
...
```
BenchmarkShmRingLargePayloads/1MB      24K ops    48 µs/op   21,653 MB/s
BenchmarkShmRingLargePayloads/4MB       5K ops   204 µs/op   20,512 MB/s
BenchmarkShmRingLargePayloads/16MB    1.5K ops   742 µs/op   22,606 MB/s
...
|-----------|-----------------|---------------|------------|
| **Shared Memory** | ~0.7 µs | ~147 ns | 7+ GB/s |
| Unix Sockets | ~9.6 µs | ~2.6 µs | 390 MB/s |
| TCP Loopback | ~18 µs | ~6.4 µs | 160 MB/s |
...
lis, err := transport.NewShmListener(
    ...
    ringASize uint64,    // Client→Server ring size
    ringBSize uint64,    // Server→Client ring size
) (*ShmListener, error)
...
type DialOptions struct {
    SegmentSize    uint64        // Total segment size
    RingASize      uint64        // Client→Server ring
    RingBSize      uint64        // Server→Client ring
