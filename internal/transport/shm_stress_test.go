...
// TestShmStreamSchedulerFairness tests that the stream scheduler provides
// fair bandwidth allocation across streams with different weights.
func TestShmStreamSchedulerFairness(t *testing.T) {
	scheduler := NewStreamScheduler()

	// Add streams with different weights
	// Stream 1: weight 32 (high priority)
	// Stream 2: weight 16 (normal priority)
	// Stream 3: weight 8 (low priority)
	priorities := []StreamPriority{
		{StreamID: 1, Weight: 32},
		{StreamID: 2, Weight: 16},
		{StreamID: 3, Weight: 8},
	}

	for _, p := range priorities {
		scheduler.AddStream(p)
		scheduler.MarkActive(p.StreamID, 1000) // All have pending data
	}

	// Run scheduler and count selections
	const iterations = 1000
	selections := make(map[uint32]int)

	for i := 0; i < iterations; i++ {
		streamID, _ := scheduler.NextStream(100)
		if streamID != 0 {
			selections[streamID]++
			// Re-mark active after being selected
			scheduler.MarkActive(streamID, 1000)
		}
	}

	t.Logf("Selections after %d iterations:", iterations)
	for id, count := range selections {
		t.Logf("  Stream %d: %d selections (%.1f%%)", id, count, float64(count)/float64(iterations)*100)
	}

	// Verify proportional allocation (with some tolerance)
	// Expected ratio: 32:16:8 = 4:2:1 = 57%:29%:14%
	totalSelections := selections[1] + selections[2] + selections[3]
	if totalSelections == 0 {
		t.Fatal("No streams were selected")
	}

	stream1Ratio := float64(selections[1]) / float64(totalSelections)
	stream2Ratio := float64(selections[2]) / float64(totalSelections)
	stream3Ratio := float64(selections[3]) / float64(totalSelections)

	// Allow 20% deviation from expected ratios
	const tolerance = 0.20
	if stream1Ratio < 0.57-tolerance || stream1Ratio > 0.57+tolerance {
		t.Errorf("Stream 1 ratio %.2f outside expected range [%.2f, %.2f]",
			stream1Ratio, 0.57-tolerance, 0.57+tolerance)
	}
	if stream2Ratio < 0.29-tolerance || stream2Ratio > 0.29+tolerance {
		t.Errorf("Stream 2 ratio %.2f outside expected range [%.2f, %.2f]",
			stream2Ratio, 0.29-tolerance, 0.29+tolerance)
	}
	if stream3Ratio < 0.14-tolerance || stream3Ratio > 0.14+tolerance {
		t.Errorf("Stream 3 ratio %.2f outside expected range [%.2f, %.2f]",
			stream3Ratio, 0.14-tolerance, 0.14+tolerance)
	}
}

// TestShmBDPEstimation tests the BDP estimator produces reasonable estimates.
func TestShmBDPEstimation(t *testing.T) {
	var lastUpdate uint32
	updateFn := func(n uint32) {
		lastUpdate = n
	}

	est := newShmBDPEstimator(65536, updateFn)

	// Simulate data flow with known timing
	// Add some bytes
	shouldPing := est.add(1000)
	if !shouldPing {
		t.Error("Expected first add to trigger BDP ping")
	}

	// Record ping time
	est.timesnap()

	// Simulate some delay (RTT)
	...
	// BDP should be estimated based on bytes received during RTT
	// We sent 2000 bytes over ~10ms, so BDP estimate should be non-zero
